...
- If you need subfunctions to write a complex function, you should define these sub-
    functions asstaticto avoid publishing them with your library. It would be a good
...
```
make bonuswill add the bonus functions to the libft.a library.
```
You do not have to add_bonusto the .c files and the header in this part. Only add
_bonusto the files containing your own bonus functions.
...
```
- content: The data contained in the element. Thevoid *allows to store any kind
    of data.
- next: The next element’s address orNULLif it’s the last element.
